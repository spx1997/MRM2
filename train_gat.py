...
from models_GAT.ResNetGAT import ResNetGAT
from models_GAT.InceptionTimeGAT import InceptionTimeGAT
from models_GAT.FCNGAT import FCNGAT
from models_GAT.RNN_FCNGAT import MLSTM_FCNGAT
from models_GAT.OS_CNNGAT import OS_CNNGAT
...
if __name__ == '__main__':
    ...
    batch_size_label = min(batch_size, x_train_num)
    ...
    if train_config.model_name.lower() == 'fcn':
        model = FCNGAT(int(x_train.shape[1]), nb_classes).to(device)
    elif train_config.model_name.lower() == 'mlstm_fcn':
        model = MLSTM_FCNGAT(int(x_train.shape[1]), nb_classes, shuffle=False).to(device)
    elif train_config.model_name.lower() == 'resnet':
        model = ResNetGAT(int(x_train.shape[1]), nb_classes).to(device)
    elif train_config.model_name.lower() == 'inceptiontime':
        model = InceptionTimeGAT(int(x_train.shape[1]), nb_classes).to(device)
    else:
        ...
        model = OS_CNNGAT(layer_parameter_list, nb_classes, False).to(device)
    ...
    for epoch in range(train_config.train_from_scratch_max_epoch, max_epoch):
        ...
        if epoch > train_config.train_from_scratch_max_epoch + (
            ...
            torch.save(model.state_dict(),
                       os.path.join(out_dir, 'checkpoints', "{}_best_gats_model.pth".format(basename)))

    print('W/ GAT, The best training loss: %.6f, the test loss: %.6f, the test accuracy: %.6f' % (
