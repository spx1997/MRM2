...
from models_MRM2.ResNetMRM2 import ResNetMRM2
from models_MRM2.InceptionTimeMRM2 import InceptionTimeMRM2
from models_MRM2.FCNMRM2 import FCNMRM2
from models_MRM2.RNN_FCNMRM2 import MLSTM_FCNMRM2
from models_MRM2.OS_CNNMRM2 import OS_CNNMRM2

from utils import train_test_supervised_batch, \
    train_semi_supervised_batch, \
    readuea
...
if __name__ == '__main__':
    ...
    batch_size_label = min(batch_size_label, x_train_num)
    batch_size_unlabel = min(batch_size_unlabel, x_test_num)
    print("batch size of labeled data: {}, batch size of unlabeled data: {}".format(batch_size_label, batch_size_unlabel))
    ...
    def generate_unlabeled_batch_data(x_train_unlabeled, batch_size_unlabel):
        while True:
            idx = np.random.choice(x_train_unlabel.shape[0], min(x_train_unlabel.shape[0], batch_size_unlabel),
                                   replace=False)
            yield {
                "xs": torch.from_numpy(x_train_unlabeled[idx]).unsqueeze(1).type(torch.float32) if len(
                    x_train_unlabeled.shape) == 2 else \
                    torch.from_numpy(x_train_unlabeled[idx]).type(torch.float32),
                "is_labeled": torch.from_numpy(np.zeros(idx.shape[0]).astype(np.bool_))
            }


    unlabeled_dataloader = generate_unlabeled_batch_data(x_train_unlabel, batch_size_unlabel)
    ...
    if train_config.model_name.lower() == 'fcn':
        model = FCNMRM2(int(x_train.shape[1]), nb_classes).to(device)
    elif train_config.model_name.lower() == 'mlstm_fcn':
        model = MLSTM_FCNMRM2(int(x_train.shape[1]), nb_classes, shuffle=False).to(device)
    elif train_config.model_name.lower() == 'resnet':
        model = ResNetMRM2(int(x_train.shape[1]), nb_classes).to(device)
    elif train_config.model_name.lower() == 'inceptiontime':
        model = InceptionTimeMRM2(int(x_train.shape[1]), nb_classes).to(device)
    else:
    ...
    for epoch in range(train_config.train_from_scratch_max_epoch, max_epoch):
        start = time.time()
        train_res_dict = train_semi_supervised_batch(
            model,
            criterion,
            labeled_dataloader,
            device,
        ...
        print('epoch [%4d/ %4d], test loss: %.6f, test acc %.6f, tcrm2 loss: %.6f, tcrm2 acc: %.6f,  ttrm2 loss: %.6f, '
            'ttrm2 acc: %.6f, time: %f s' %
            (epoch + 1, train_config.max_epoch, test_res_dict['loss'],
             test_res_dict['acc'],
             train_res_dict['tcrm2_loss'], train_res_dict['acc_tcrm2'], train_res_dict['ttrm2_loss'],
             train_res_dict['acc_ttrm2'], time.time() - start))
        scheduler.step(train_res_dict['tcrm2_loss'])
        if epoch > train_config.train_from_scratch_max_epoch + (
                ...
                train_res_dict['tcrm2_loss']:
            best_loss = train_res_dict['tcrm2_loss']
            final_res = [test_res_dict['loss'], test_res_dict['acc']]
            torch.save(model.state_dict(),
                       os.path.join(out_dir, 'checkpoints', "{}_best_mrm2_model.pth".format(basename)))

    print('W/ MRM2, The best training loss: %.6f, the test loss: %.6f, the test accuracy: %.6f' % (
    ...
    print('alpha: {}, beta: {}'.format(train_config.alpha, train_config.beta))
